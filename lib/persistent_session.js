...
// Helper functions to replace store
var store = {
  get: function(key) {
...
PersistentSession = function (dictName) {
  ...
  function migrateToEJSON() {
    if (localStorage.getItem('__PSDATAVERSION__' + self._dictName) >= 1) {
      return;
    }

    var psKeyList = JSON.parse(store.get('__PSKEYS__' + self._dictName) || '[]');
    var psaKeyList = JSON.parse(store.get('__PSAKEYS__' + self._dictName) || '[]');

    _.each([psKeyList, psaKeyList], function(list) {
      _.each(list, function(key) {
        store.set(key, EJSON.stringify(store.get(key)));
      });
    });

    store.set('__PSDATAVERSION__' + self._dictName, 2);
  };

  function migrate3Xto4X() {
    if (store.get('__PSDATAVERSION__' + self._dictName) >= PSA_DATA_VERSION) {
      return;
    }
    ...
    _.each([psKeyList, psaKeyList], function(list) {
      _.each(list, function(key) {
        var invalid = false;
        try {
          EJSON.parse(store.get(self._dictName+key));
        } catch (error) {
        ...
        if (!invalid) {
          var parsed = EJSON.parse(store.get(self._dictName+key));
          var jsoned = EJSON.toJSONValue(parsed);
          store.set(self._dictName+key, jsoned);
        }
      });
    });

    store.set('__PSDATAVERSION__' + self._dictName, 4);
  }
  ...
  if (Meteor.isClient) {
    ...
    Meteor.startup(function(){
      ...
      // persistent data
      var psList = JSON.parse(store.get('__PSKEYS__' + self._dictName) || '[]');
      ...
      // authenticated data
      var psaList = JSON.parse(store.get('__PSAKEYS__' + self._dictName) || '[]');
...
PersistentSession.prototype.store = function _psStore(type, key, value) {
  ...
  this.psKeyList  = JSON.parse(store.get('__PSKEYS__' + this._dictName) || '[]');
  this.psaKeyList = JSON.parse(store.get('__PSAKEYS__' + this._dictName) || '[]');

  if (type == 'get') {
    return store.get(this._dictName + key);
  } else {
    ...
    store.set('__PSKEYS__', JSON.stringify(this.psKeyList));
    store.set('__PSAKEYS__', JSON.stringify(this.psaKeyList));
    store.set(this._dictName + key, EJSON.toJSONValue(value));
  }
};
